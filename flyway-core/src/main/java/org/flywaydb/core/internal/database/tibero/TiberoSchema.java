 ...
 */
package org.flywaydb.core.internal.database.tibero;
...
import static org.flywaydb.core.internal.database.tibero.TiberoSchema.ObjectType.*;

/**
 * Oracle implementation of Schema.
 */
public class TiberoSchema extends Schema<TiberoDatabase, TiberoTable> {
    private static final Log LOG = LogFactory.getLog(TiberoSchema.class);
     ...
     */
    TiberoSchema(JdbcTemplate jdbcTemplate, TiberoDatabase database, String name) {
    ...
    protected TiberoTable[] doAllTables() throws SQLException {
        ...
        TiberoTable[] tables = new TiberoTable[tableNames.size()];
        for (int i = 0; i < tableNames.size(); i++) {
            tables[i] = new TiberoTable(jdbcTemplate, database, this, tableNames.get(i));
        }
        return tables;
    }
    ...
    public enum ObjectType {
        ...
        TABLE("TABLE", "CASCADE CONSTRAINTS PURGE") {
            @Override
            public List<String> getObjectNames(JdbcTemplate jdbcTemplate, TiberoDatabase database, TiberoSchema schema) throws SQLException {
        ...
        QUEUE_TABLE("QUEUE TABLE") {
            ...
            @Override
            public String generateDropStatement(JdbcTemplate jdbcTemplate, TiberoDatabase database, TiberoSchema schema, String objectName) {
        ...
        DATABASE_LINK("DATABASE LINK") {
            @Override
            public void dropObjects(JdbcTemplate jdbcTemplate, TiberoDatabase database, TiberoSchema schema) {
         ...
         */
        public void dropObjects(JdbcTemplate jdbcTemplate, TiberoDatabase database, TiberoSchema schema) throws SQLException {
         ...
         */
        public static Set<String> getObjectTypeNames(JdbcTemplate jdbcTemplate, TiberoDatabase database, TiberoSchema schema) throws SQLException {
            boolean xmlDbAvailable = database.isXmlDbAvailable();
...
//                                            "SELECT * FROM ALL_SCHEDULER_CREDENTIALS WHERE OWNER = ?) "
            ...
            // changed from 6 to 3 by luis kim
            int n = 3 + (xmlDbAvailable ? 1 : 0) +
         ...
         * @throws SQLException if retrieving of object types failed.
         */
        public static boolean supportedTypesExist(JdbcTemplate jdbcTemplate, TiberoDatabase database, TiberoSchema schema) throws SQLException {
